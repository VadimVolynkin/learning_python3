...
Питон знает к какому классу отнсится объект и выбирает для него соответствующий метод при обращении к одинаковому имени метода.
...
@property - cвойства экземпляра класса - тип атрибута, который вычисляется только 1 раз(по сути функция), но обращение к нему без () как к простому атрибуту.
...
issubclass(sub_class, class)                    # проверят является ли подклассом
...
class SomeClass():
    __Foo = 5              # _Classname__Foo

    def bar():
        return __some()    # _Classname__some()
