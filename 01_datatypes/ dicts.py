...
# Ключ - hashable объект неизменяемого типа(строка, число, кортеж), который не меняется.
# вес - 48 байт
...
dict.copy() - возвращает копию словаря - новый объект с новыми значениями
...
sorted_by_key = {key: val for key, val in sorted(num_dict.items(), key=lambda x:x[1])}
print(sorted_by_key)


sorted_by_key = {key: val for key, val in num_dict.items() if val >= 30}
print(sorted_by_key)
