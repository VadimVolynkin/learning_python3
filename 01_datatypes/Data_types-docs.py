...
@staticmethod - это обычная функция, в пространстве имен класса. Нет self - не оперирует экземплярами.  Может быть вызван относительно экземпляра.

@property - обращения к методу, как к простому атрибуту без круглых скобок ().
Свойства экземпляра класса - тип атрибута, который вычисляется только 1 раз(по сути функция), но обращение к нему без ().
...
Неизменяемые типы данных: строки, все числа, кортежи, неизменяемые множества, None
