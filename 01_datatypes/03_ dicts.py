...
# после Питона 3.7 словари стали упорядоченными. Хранятся не хеши и значения, а значения и ссылки на список с хешем. 
...
rev_dict = reversed(dict)     # Python 3.8+ Вернет итератор с обратным порядком по ключам
...
dict['key']                   # вернет значение, если нет такого ключа - KeyError
...
key in dict                   # проверит наличие ключа
key in dict.keys()
